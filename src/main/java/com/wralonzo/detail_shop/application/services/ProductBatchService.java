...
import com.wralonzo.detail_shop.domain.entities.*;
...
public class ProductBatchService {
  ...
  private void setProductRelations(Product product, Row row) {
    ...
    Long catId = (long) getCellValueAsDouble(row.getCell(7));
    ...
    Long supId = (long) getCellValueAsDouble(row.getCell(8));
    if (supId > 0) {
      product.setSupplier(supplierRepository.findById(supId)
          .orElseThrow(() -> new RuntimeException("Proveedor ID " + supId + " no encontrado.")));
    }

    // ALMACÉN (Opcional) - Columna 9
    Long warId = (long) getCellValueAsDouble(row.getCell(9));
    if (warId > 0) {
      product.setWarehouse(warehouseRepository.findById(warId)
          .orElseThrow(() -> new RuntimeException("Almacén ID " + warId + " no encontrado.")));
    }
  }
  ...
  public byte[] generateTemplate() throws Exception {
    try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      String[] columns = { "Nombre", "SKU", "Descripción", "Precio Compra", "Precio Venta", "Stock Mínimo",
          "Código de barras", "ID Categoría", "ID Proveedor", "ID Almacén" };
